...
func TestSMTPMailer_LocalizedSubjects(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			expectedSubject: "Subject: Votre code d'authentification Mail Studio",
		},
		{
			...
			expectedSubject: "Subject: Vous avez été invité à rejoindre Mon Espace sur Mail Studio",
		},
		{
...
func TestSMTPMailer_UnknownLanguageFallsBackToEnglish(t *testing.T) {
	...
	if !strings.Contains(logOutput, "Subject: Your Mail Studio authentication code") {
...
func TestConsoleMailer_LocalizedContent(t *testing.T) {
	...
	expectedStrings := []string{
		...
		"Bonjour,",
		"Votre code d'authentification pour Mail Studio est :",
		"123456",
		"L'équipe Mail Studio",
	}
	for _, expected := range expectedStrings {
		if !strings.Contains(output, expected) {
